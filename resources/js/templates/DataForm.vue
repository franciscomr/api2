...
export default {
  ...
  setup(props) {
    ...
    return {
      form, relationships, errors, submit, submitCreate, submitUpdate
    }
  },
}
</script>
      ...
      <form @submit.prevent="submit(new_record)">
        <div class="space-y-3">
          <div v-for="(field, index) in fields" :key="index">
            ...
            <FormSelect v-if="field.type === 'select'" :name="relationships[field.name]"
              v-model="relationships[field.name]" :id="field.name" :label="field.label" :values="field.propieties.value"
              :message="errors[field.name]" />
          </div>
